        ...
        try {
            ...
            if (user) {
                ...
                if (!isNumber(user.iron))
                    ...
                    //Peces
                if (!isNumber(user.blowfish)) 
                user.blowfish = 0
                if (!isNumber(user.tropicalfish)) 
                user.tropicalfish = 0
                if (!isNumber(user.commonfish)) 
                user.commonfish = 0
                if (!isNumber(user.crab)) 
                user.crab = 0
                if (!isNumber(user.locust)) 
                user.locust = 0
                ...
                if (!isNumber(user.dogexp))
                    user.dogexp = 0
                if (!isNumber(user.loro))
                    user.loro = 0
                if (!isNumber(user.loroexp))
                    user.loro = 0
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `⚠️️ *_Nivel requerido ${plugin.level} para usar este comando._* \nTu nivel ${_user.level}`, m)
...
export async function participantsUpdate({ id, participants, action }) {
                ...
                for (let user of participants) {
                    ...
                    let about = (await this.fetchStatus(user).catch(console.error) || {}).status || '×'
                        ...
                        [(action == 'add' ? 'Gracias ❤' : 'Adios 👋'), 'ura'],    
                            ['Menú 📒', '/menu']
                            ], fgclink, { contextInfo: { externalAdReply: { showAdAttribution: false, title: '𝗦𝗵𝗶𝗻𝗮⚔𝐵𝑂𝑇࿐ Sᴜᴘᴘᴏʀᴛ❐🎋༉', body: global.botname, sourceUrl: global.linkgc, thumbnail: miniurl }}, mentions: [user]})
                    }
                }
            }
            break
  ...
  let bio = biot.status?.toString() || 'Sin Info'
            if (chat.detect)
                this.sendButton(id, text, saludo, pp, [['Menú 📒', '/menu']], false, { contextInfo: { externalAdReply: { showAdAttribution: false, title: '𝗦𝗵𝗶𝗻𝗮⚔𝐵𝑂𝑇࿐ Sᴜᴘᴘᴏʀᴛ❐🎋༉', body: global.botname, sourceUrl: global.linkgc, thumbnail: miniurl }}, mentions: this.parseMention(text) })
            break
            }
            }
            }
...
let date = d.toLocaleDateString('es', { day: 'numeric', month: 'long', year: 'numeric' })
 let time = d.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true })
    try {
        ...
        if (chat.delete)
            return
        await conn.relayMessage(msg.chat, { extendedTextMessage: { text: `┏━━━━⬣  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀  ⬣━━━━⬣\n┃🐢 *❏ Nombre:* @${participant.split`@`[0]}\n┃🕒 *❏ Hora:* ${time}\n┃📅 *❏ Fecha:* ${date}\n┃🥗 *❏ Enviando mensaje...*\n┗━━━━━━━━━━━━━━━━━⬣`, contextInfo: { externalAdReply: { title: '𝗦𝗵𝗶𝗻𝗮⚔𝐵𝑂𝑇࿐ Sᴜᴘᴘᴏʀᴛ❐🎋༉', body: '', thumbnail: verificado, sourceUrl: linkgc }, mentionedJid: [participant] }}}, { quoted: fakemsg })
...
global.dfail = (type, m, conn) => {
    ...
    let msg = {
        rowner: '⚠️️ *_Este comando solo puede ser utilizado por el -Creador del bot-_*',
        owner: '⚠️ *_Este comando solo puede ser utilizado por el -Dueño del Bot-_*',
        mods: '⚠️ *_Esta función es solo para - moderadores del Bot-_*',
        premium: '⚠️ *_Este comando es solo para miembros -Premium-_*',
        group: '⚠️ *_¡Este comando solo se puede usar en grupos!_*',
        private: '⚠️️ *_Este comando solo se puede usar en el chat -privado del Bot-_*',
        admin: '⚠️️ *_Este comando es solo para -Admins- del grupo_*',
        botAdmin: '⚠️️ *_¡Para usar este comando debo ser -Administrador!-_*',
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '𝗦𝗵𝗶𝗻𝗮⚔𝐵𝑂𝑇࿐', body: '', thumbnail: miniurl, sourceUrl: script }}}}, { quoted: m })
