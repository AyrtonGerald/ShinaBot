...
if (/^@5492266466080|@56964787183|@5492266613038$/i.test(m.text) ) { //sem prefixo
    conn.reply(m.chat, `❗No etiquetes a mi creador si tiene alguna consulta o dudas, hablarle el pv solo por tema del bot`, m) //wm, null, [['Menu', '#menu']], m) botones :V

}

if (/^Mande porno|porno|paja$/i.test(m.text) ) { //sem prefixo
    let teks = `
${pickRandom([`_haci te queria hagarrar_ 🧐`, `_Quita mierda_`, `_mis admins te pueden eliminar_`, `_busca trabajl mejor_`, `_estas enfermo_`, `_a buscar eso en otro lado_`, `_te advierto que esta prohibido_`, `_quieres que te elimine?_`, '_seras eliminado_'])}
`.trim()
